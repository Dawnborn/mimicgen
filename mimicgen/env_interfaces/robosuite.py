...
class MG_BinPicking(RobosuiteInterface):
    """
    Corresponds to robosuite PickUp task and variants.
    """
    def get_object_poses(self):
        """
        Gets the pose of each object relevant to MimicGen data generation in the current scene.
        ...
        # two relevant objects - milk and cereal
        return dict(
    ...
    def get_subtask_term_signals(self):
        ...
        # first subtask is grasping milk (motion relative to milk)
        signals["grasp"] = int(self.env._check_grasp(
            gripper=self.env.robots[0].gripper,
            object_geoms=[g for g in self.env.target.contact_geoms])
        )

        # final subtask is placing milk into cereal (motion relative to cereal) - but final subtask signal is not needed
        return signals
